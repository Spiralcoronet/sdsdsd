...
function InterfaceTracker(bounds, mainimgsrc) {
	...
	this.refind = function (imgref) {
		if (this.pos) {
			testbuf = imgref.toData(this.pos.x - this.bounds.x, this.pos.y - this.bounds.y, this.mainbuffer.width, this.mainbuffer.height);
...
PasteInput = new (function () {
	...
	this.start = function () {
		...
		var apipasted = function (e) {
			var a;
			for (a = 0; a < e.clipboardData.items.length; a++) {//loop all data types
...
ImageData.fixPngBuffer = function (data) {
	...
	while (i < data.length) {
		...
		if (chunkname == "sRGB") {
			...
			qw("- Changed sRGB, old value:", data[i]);
			//new chunk data
			data[i] = 3;
			//new chunk checksum
