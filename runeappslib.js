...
function newdraghandler2(mousedownevent, movefunc, endfunc, mindist) {
	...
	var mouseloc = {
		...
		dx: 0,
		dy: 0,
		sx: mousedownevent.pageX,
		sy: mousedownevent.pageY
	};

	var moved = function (e, end) {
		if (locked && Math.abs(e.pageX - mouseloc.sx) + Math.abs(e.pageY - mouseloc.sy) >= mindist) {
			locked = false;
		}
		if (!locked) {
			mouseloc.dx = e.pageX - mouseloc.x;
			mouseloc.dy = e.pageY - mouseloc.y;
			mouseloc.x = e.pageX;
			mouseloc.y = e.pageY;
	...
	var mousemove = function (e) { moved(e, false); };
	var mouseup = function (e) {
		moved(e, true);
...
function newdraghandler(movefunc, endfunc) {
	if (draghandler) {//clear old draghandler
...
function bindAutoComplete(el, func) {
	...
	el.addEventListener("keydown", function (e) {
		...
		if (e.keyCode == 13 && scope.suggests && scope.suggests.length > 0) { scope.insertSuggestion(); e.preventDefault(); e.stopImmediatePropagation(); }
